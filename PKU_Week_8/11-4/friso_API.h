...
#define new_string_buffer() \
    new_string_buffer_with_opacity( __DEFAULT_ARRAY_LIST_OPACITY__ ); //默认初始化buffer
FRISO_API string_buffer_t new_string_buffer_with_opacity( uint_t ); //以给定大小（字节）初始化buffer
FRISO_API string_buffer_t new_string_buffer_with_string( fstring str ); //以给定字符串初始化buffer
 ...
 */
FRISO_API void string_buffer_append( string_buffer_t, fstring ); //向buffer中添加字符串
FRISO_API void string_buffer_append_char( string_buffer_t, char ); //向buffer中添加字符

//insert the given fstring from the specified position.
FRISO_API void string_buffer_insert( string_buffer_t, uint_t idx, fstring ); //未被实现！

//remove the char in the specified position.
FRISO_API fstring string_buffer_remove( string_buffer_t, uint_t idx, uint_t ); //在指定位置删除特定长度字符串
