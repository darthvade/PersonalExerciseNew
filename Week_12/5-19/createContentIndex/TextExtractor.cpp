...
void assign_regex()
{
	...
	_css			= boost::regex ("<style>[/s/S]*</style>", boost::regex::icase|boost::regex::perl); //匹配CSS标签时，原正则表达式会有BUG！
...
// added by KevinMatrix
std::string parseString(std::string &inputstr, int threshold) {
	assign_regex();
	
	removeTags(inputstr);
	statistic_line(inputstr);
	statistic_block(3);
